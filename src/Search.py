...
def search_in_reversed_index(expression):
    def parse_expression(expression):
        ...
        expression = re.sub(r'\b([a-zA-Z]+)\b', lambda m: m.group(1).lower() if m.group(1) not in ['AND', 'OR'] else m.group(1), expression)
        
        # Replace words with trie.search("word")
        expression_with_trie = re.sub(r'\b([a-z0-9]+)\b', r'trie.search("\1")', expression)
        ...
        def extract_snippet_from_file(file_path, searched_words):
            try:
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    pattern = re.compile(r'\b(' + '|'.join(re.escape(word) for word in re.findall(r'\b[a-zA-Z0-9]+\b', searched_words)) + r')\b', re.IGNORECASE)
                    match = pattern.search(content)
                    content = pattern.sub(r'<span class="highlight">\1</span>', content)
                    if(match):
        ...
        def find_z_score(file_path, searched_words):
            try:
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    words = re.findall(r'\b[a-zA-Z0-9]+\b', searched_words.lower())
